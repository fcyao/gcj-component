  ...
  methods: {
    ...
    getOptions(chartData) {
      ...
      if (chartData && chartData.length) {
        xSeries = chartData.map(item => item.name)
        ySeries = chartData.map(item => item.value)
      } else {
        return {
          title: {
            text: this.title
      ...
      let xName = this.units.length ? this.units[1] : ''
      ...
      const option = {
        ...
        tooltip: {
          ...
          formatter: (params) => {
            ...
            if (this.title === '渠道/平台使用偏好') {
              xName = ''
            }
            let res = params[0].name + xName + ':' + RetainedDecimalPlaces(params[0].data, 0)
            res = res + '(' + ((params[0].data / count) * 100).toFixed(2) + '%)'
            return res
          }
        },
        ...
        xAxis: {
          name: this.units.length ? this.units[1] : '',
          type: 'category',
          axisLabel: {
            interval: 0
          },
        ...
        yAxis: {
          name: this.units.length ? this.units[0] : '',
          type: 'value'
        },
        aria: {
          enabled: true,
